...
本文向你展示10个Ruby技巧（或许你都知道），但不管怎样，这些都是值得分享的东西。
...
定义一个lambda可以使用在Rails中也用的比较多的`->`字面量，它可以很容易的定义一个 lambda 对象
...
有时你可能会先给出一个选项去接受单个对象或者一个数组对象，而不是最后去检查你接受到对象的类型，这是你可以用 `[*type]` 或者 `Array(type)`
...
大多数都认为它和下面的表示方式一样，但实际不是

{% highlight ruby %}
a = a || b # 错误
{% endhighlight %}

如果 a 已经存在，第二种方法的表示就在给a进行分配赋值，这样是没有意义的。

#### **6.强制的哈希参数**

这个是在 Ruby 2.0 被引进的，你可以在定义方法时指定接受的参数为hash类型，像这样：
...
你可能想要生成一个数字列表或者把所有的字符放入一个数组，你可以用 Ruby 的 ranges 去做这样的事：
...
默认的，当我们试图接受一个在 hash 中未定义的值，你将会得到 nil。你可以改变这个结果在初始时。
...
但是这有一个技巧可以避免它，通过用 `gsub` 方法加一个正则表达式。你可以自动的去除前面的空格，这样你就能保持缩进。
